...
description: 'To implement a structured logging system that outputs JSON logs for machine parsing (in production) and readable text (in development), aiding in debugging and monitoring. Use when: When replacing console.log statements; When setting up observability tools (ELK stack, Datadog, etc.); When needing different log levels (INFO, WARN, ERROR).'
---
