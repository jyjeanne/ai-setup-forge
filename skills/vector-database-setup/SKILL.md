...
description: 'To provision and configure a vector database (Vector DB) for storing high-dimensional embeddings, enabling semantic search and RAG applications. Use when: Implementing RAG (Retrieval-Augmented Generation); Building recommendation systems based on similarity; Implementing semantic search (search by meaning, not just keywords).'
---
